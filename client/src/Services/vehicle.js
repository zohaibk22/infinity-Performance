...
export const createNewVehicle = async (vehicleData) => {
  const resp = await api.post(`/vehicles`, { vehicleData: vehicleData });
  return resp.data;
};

export const updateVehicle = async (id, vehicleData) => {
  const resp = await api.put(`/vehicles/${id}`, { vehicleData: vehicleData }); //for our strong params within vehicle controller
  return resp.data;
};
